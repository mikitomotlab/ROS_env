...
#define structure pos value
    def call_service(self, th, dt, dth, dx, dy, x, y, st, sr):
        ...
        return th, dt, dth, dx, dy, x, y, st, sr
    
#call service and go into get()
    def server(self, i):
        rospy.init_node('setvel_server')
        #th, dt, dth, dx, dy, st, sr, i = self.set_value(i)
        th, dt, dth, dx, dy, x, y, st, sr = self.call_service(self.th, self.dt, self.dth, self.dx, self.dy, self.x, self.y, self.start_t, self.start_r)
        print "th: {}".format(self.th)
        print "dx: {}".format(self.dx)
        print "dy: {}".format(self.dy)
...
if __name__ == "__main__":
    i = 0
    global th, dt, dth, dx, dy, x, y, st, sr
    th, dt, dth, dx, dy, x, y, st, sr = zeroset()
    d_pos = pos(th, dt, dth, dx, dy, x, y, st, sr) 
    d_pos.server(i)
