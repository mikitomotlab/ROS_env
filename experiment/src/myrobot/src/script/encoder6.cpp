...
// コールバック関数。並進、回転速度の表示。
void cbVel(const geometry_msgs::Twist::ConstPtr& vel, struct name d_pos[]) {
...
void cbOdom(const nav_msgs::Odometry::ConstPtr& msg, struct name d_pos) { 
    ...
    time_t now;
    time_t finish;
    double t_s = msg->twist.twist.linear.x;
    double r_s = msg->twist.twist.angular.z;
    double ltime = 0;
    double rtime;
    double ftime;
    double pi = 3.14;
    ...
    if (t_s >= 0.09){
      ...
      ftime = now - d_pos.start_t;
      cout << "forword_time :" << ftime << endl;
      cout << "start_t :" << d_pos.start_t << endl;
      cout << "start_r :" << d_pos.start_r << endl;
    }
    if (r_s >= 0.09){
      if (d_pos.start_t != 0){
        time(&d_pos.start_r);
